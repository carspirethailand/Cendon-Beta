...
**วิธีนำเข้า/อัปเดตเนื้อหา** — แผงควบคุม → สอน AI → นำเข้าเป็นชุด แล้ววางไฟล์ที่แบ่งตอนแล้ว
หรือเรียกเอง

```bash
curl -X POST "$BASE/api/kb/bulk" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  --data-binary @handbook-parts.json
```

รูปแบบแต่ละตอน — `enabled` ต้องเป็น `0` และ `id` ต้องขึ้นต้นด้วย `hb_`

```json
{ "id":"hb_02", "title":"ภาค 2 — D1", "body":"...มาร์กดาวน์ของภาคนี้...",
  "keywords":"handbook", "make":"__handbook__", "model":"", "enabled":0 }
```
