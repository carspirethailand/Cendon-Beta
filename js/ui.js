...
// Update home view welcome message according to the hour
export function updateGreeting() {
  const el = $("greet");
  if (!el) return;
  const h = new Date().getHours();
  const g = h < 11 ? "สวัสดีตอนเช้า" : h < 16 ? "สวัสดีตอนบ่าย" : h < 19 ? "สวัสดีตอนเย็น" : "สวัสดีตอนค่ำ";
  const em = h < 11 ? "☀️" : h < 16 ? "🌤️" : h < 19 ? "🌇" : "🌙";
  const name = state.currentUser ? state.currentUser.name : "";
  el.innerHTML = `<span class="g-em">${em}</span> ${g}${name ? ", คุณ " + name : ""} <span class="g-wave">👋</span>`;
}
...
export function revealActive() {
  ...
  if (!v) return;
  v.querySelectorAll(".greet,.billboard,.hub,.quiz,.section-title,.section-sub,.mag-feature,.brief-bar,.phead,.subtabs,.diag-layout,.settings-grid,.shop-grid,.mag-grid").forEach(el => {
